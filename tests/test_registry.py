"""Tests for clodbox.registry: OCI digest client (all network mocked)."""
...
class TestParseImageRef:
    def test_standard(self):
        reg, repo, tag = _parse_image_ref("ghcr.io/doctorjei/clodbox-base:latest")
        assert reg == "ghcr.io"
        assert repo == "doctorjei/clodbox-base"
        assert tag == "latest"
...
class TestGetAnonymousToken:
    def test_ghcr_returns_token(self):
        ...
        with patch("clodbox.registry.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_url_construction(self):
        ...
        with patch("clodbox.registry.urllib.request.urlopen", return_value=mock_resp) as m:
            _get_anonymous_token("ghcr.io", "doctorjei/clodbox-base")
            req = m.call_args[0][0]
            assert "scope=repository:doctorjei/clodbox-base:pull" in req.full_url
...
class TestGetRemoteDigest:
    def test_success(self):
        with (
            patch("clodbox.registry._parse_image_ref", return_value=("ghcr.io", "o/r", "latest")),
            patch("clodbox.registry._get_anonymous_token", return_value="tok"),
            patch("clodbox.registry._fetch_manifest_digest", return_value="sha256:abc"),
        ):
            assert get_remote_digest("ghcr.io/o/r:latest") == "sha256:abc"

    def test_network_error(self):
        with patch("clodbox.registry._parse_image_ref", side_effect=OSError("timeout")):
